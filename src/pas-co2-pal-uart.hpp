...
class UARTPAL
{
    public:
         ...
         * @param[in]   dataBits    Number of data bits transmitted between start and stop conditions
         ...
         * @param[in,out]   length      Data length in bytes. Outputs the actual number of written bytes
         * @return          PAS CO2 error code
         * @retval          OK if success
         * @retval          INTF_ERROR if error
         */
        virtual Error_t     write   (const uint8_t * data, uint32_t & length) = 0;

        /**
         * @brief           Reads from the UART interface
         * @param[in]       *data       Pointer to the array that will store the read data
         * @param[in,out]   length      Data length in bytes. Outputs the actual number of read bytes
         * @return          PAS CO2 error code
         * @retval          OK if success
         * @retval          INTF_ERROR if error
         */
        virtual Error_t     read    (       uint8_t * data, uint32_t & length) = 0;
};
